...
function serializeUser(doc) {
  ...
  : [],
    ...
    pinnedQuoteBookId: data.pinnedQuoteBookId || null,
    pinnedQuoteText: data.pinnedQuoteText || null,
...
export async function ensureUserProfile(decodedToken) {
  ...
  await getDb().runTransaction(async (transaction) => {
    ...
    const next = {
      email: decodedToken.email || current.email || "",
      displayName: decodedToken.name || current.displayName || "",
      photoURL: decodedToken.picture || current.photoURL || "",
      provider:
        decodedToken.firebase?.sign_in_provider || current.provider || "",
    };
  ...
  profileCache.delete(decodedToken.uid);
}
...
export async function getUserProfile(uid) {
  ...
  const profile = serializeUser(doc);
...
// `bookId` null clears the pick — the profile falls back to auto-picking
// a book with a description again. `quoteText` is the exact sentence the
// user chose from that book's description, so two people quoting the same
// book don't end up with whatever sentence happens to come first.
export async function updatePinnedQuoteBook(uid, bookId, quoteText) {
  await getDb()
    .collection(COLLECTION)
    .doc(uid)
    .update({
      pinnedQuoteBookId: bookId ? String(bookId) : null,
      pinnedQuoteText: bookId && quoteText ? String(quoteText) : null,
      updatedAt: FieldValue.serverTimestamp(),
    });
}