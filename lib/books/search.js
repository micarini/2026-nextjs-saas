...
import { searchHardcover } from "@/lib/books/providers/hardcover";
...
// Hardcover leads — better, more current covers than Google Books or Open
// Library tend to have. Both it and Google Books are fired in parallel so
// falling back never costs a second round-trip's worth of latency; Open
// Library is the last resort, tried only if neither of those found
// anything.
export async function searchBookCovers(query) {
  const [hardcoverResults, googleResults] = await Promise.all([
    searchHardcover(query).catch(() => []),
    searchGoogleBooks(query),
  ]);

  if (hardcoverResults.length > 0) {
    return rankEditions(hardcoverResults);
  }
