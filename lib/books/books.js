...
export async function updateUserBookStatus(userId, bookId, status) {
  const docRef = getDb().collection(COLLECTION).doc(bookId);
  const doc = await docRef.get();
...
export async function updateUserBookRating(userId, bookId, rating) {
  const docRef = getDb().collection(COLLECTION).doc(bookId);
  const doc = await docRef.get();
...
export async function updateUserBookDates(userId, bookId, { startDate, finishDate, targetDate }) {
  const docRef = getDb().collection(COLLECTION).doc(bookId);
  const doc = await docRef.get();
...
export async function updateUserBookSpotifyUrl(userId, bookId, spotifyUrl) {
  const docRef = getDb().collection(COLLECTION).doc(bookId);
  const doc = await docRef.get();
...
export async function updateUserBookProgress(userId, bookId, currentPage) {
  const docRef = getDb().collection(COLLECTION).doc(bookId);
  const doc = await docRef.get();
