...
function serializeBook(doc) {
  ...
  const book = {
    ...
    genre:
      data.genre ||
      (Array.isArray(data.genres) && data.genres[0]) ||
      "other",
    ...
    readingDays:
      Array.isArray(data.readingDays)
        ? data.readingDays.filter(
            (entry) =>
    ...
    readingLogs:
      Array.isArray(data.readingLogs)
        ? data.readingLogs
            .map((entry) => ({
              startDate: serializeDate(entry.startDate),
              finishDate: serializeDate(entry.finishDate),
              rating: normalizeNumber(entry.rating),
            }))
...
export async function updateUserBookStatus(
  ...
  if (status === "read") {
    ...
    /*
     * Si sabemos cuántas páginas tiene,
     * al terminarlo el progreso pasa
    ...
    /*
     * Si todavía no tiene fecha de
     * finalización, usamos el momento
     * en que lo marcó como terminado.
     *
     * Esto también ayuda a las stats
     * mensuales.
     */
...
export async function updateUserBookProgress(
  ...
  const totalPages = normalizeNumber(doc.data().totalPages);

  if (page !== null && (!Number.isInteger(page) || page < 0)) {
    throw new Error("The current page must be a whole number of zero or more.");
  }

  if (page !== null && totalPages !== null && page > totalPages) {
    throw new Error(`The current page cannot be greater than ${totalPages}.`);
  }
  ...
  const currentStatus = String(doc.data().status || "")
    .trim()
    .toLowerCase()
    .replace(/[\s-]+/g, "_");
  ...
  if (
    page > 0 &&
    ["to_read", "want_to_read"].includes(currentStatus)
  ) {
