...
async function fetchNewReleases(limit) {
  ...
  try {
    ...
    throw new Error(`NYT Books request failed: ${err?.name || "network error"}`);
  }

  if (!response.ok) {
    throw new Error(`NYT Books request failed: ${response.status}`);
  }
...
export async function getNewReleases(limit = 8) {
  try {
    return await getCachedNewReleases(limit);
  } catch {
    return [];
  }
}
